...
/** Format a 0-1 ratio as a percent string with sign. Null/NaN becomes a dash. */
export function fmtPct(value: number | null | undefined, decimals = 2): string {
  if (isNullish(value)) return PLACEHOLDER;
  return `${(value * 100).toFixed(decimals)}%`;
}

/** Format a number with fixed decimals. Null/NaN becomes a dash. */
...
/** Format a number compactly, e.g. 1_200_000 -> "1.2M". Null/NaN becomes a dash. */
